...
function initList () {
  var baseUrl = $('#baseUrl').data('base-url'),
      ...
      longest = findLongest(locations, 'text').text.length,
      targetLength = 12,
      elemWidth;

  longest = longest > targetLength ? longest : targetLength;
  elemWidth = 100 / (longest + 1.5);

  $(locations).each(function (_i, location) {
      for (var i = 0; i < longest; i++) {
        (function elem() {
          var elem, svgContainer;
          var char = location.text[i];
          char = char ? char.toUpperCase() : '';

          elem = $('<div class="outer-letter" />');
